...
export default {
  ...
  data() {
    return {
      ...
      //
      entitiesFields: [
        {key: 'select', label: '', sortable: false, thStyle: 'width: 50px'},
  ...
  methods: {
    ...mapActions({
      createNotification: 'dialogs/notification/create',
      editNotification: 'dialogs/notification/edit',
    }),
    ...
    fetchNotifications() {
      this.loading = true;
      this.$api.notifications.fetch(this.currentPage, this.params).then(response => {
    ...
    remove() {
      this.$api.notifications.delete(this.deletableId).then(response => {
    ...
    bulkDelete() {
      this.$api.notifications.bulkDelete(this.ids).then(() => {
        this.ids = [];
        this.fetchNotifications();
      });
    },
